...
export function writeMessage(
  response: Buffer,
  writable: Stream.Writable = stdout
): void {
...
export function toBufferedMessage(response: unknown): Buffer {
...
export function readMessage<T>(
  readable: Stream.Readable = stdin
): Promise<T | null> {
  return new Promise((resolve, reject) => {
    ...
    readable.once("close", onErrorOrClose);
    readable.once("end", onErrorOrClose);
    readable.once("error", onError);

    const cleanup = () => {
      readable.removeListener("close", onErrorOrClose);
      readable.removeListener("end", onErrorOrClose);
    ...
    const readLength = () => {
      ...
      const chunk = readable.read(4 - bytesRead) as Buffer | null;
      if (chunk === null) {
        readable.once("readable", readLength);
        return;
      }
    ...
    const readMessageBody = (length: number) => {
      ...
      const readBody = () => {
        ...
        const chunk = readable.read(length - messageBytesRead) as Buffer | null;
        if (chunk === null) {
          readable.once("readable", readBody);
          return;
        }
